...
public class BottomQuickActivator extends IOperator {
	...
	public BottomQuickActivator() {
		...
		enableGroup = ButtonMapping.B_ENT_L;
		disableGroup = ButtonMapping.B_CLEAR.or(ButtonMapping.B_ENT_L);
		disableFastGroup = ButtonMapping.B_ENT_R;
	}
	...
	public void update(Model model) {
		...
		long deltaT = now - timestamp;
		boolean enable = enableGroup.evaluate(model);
		boolean disable = disableGroup.evaluate(model);
		boolean disableNow = disableFastGroup.evaluate(model);

		switch (state) {
		case BASE:
			if (enable) {
				state = State.BASE_ARMED;
				timestamp = now;
			}
			break;
		case BASE_ARMED:
	...
	private void enableQM(Model model) {
		IOperator base_op = Registries.operators.find(o -> o.getClass().equals(BottomOperator.class));
		IOperator quic_op = Registries.operators.find(o -> o.getClass().equals(BottomQuickMode.class));
		if (quic_op == null) {
			logger.warning("Quick-Mode transition not possible, since QM operator is missing");
			return;
		}
		if (base_op == null) {
			logger.severe("Quick-Mode transition not possible, since Base operator is missing");
			return;
		}
	...
	private void disableQM(Model model, boolean quick) {
		...
		IOperator quic_op = Registries.operators.find(o -> o.getClass().equals(BottomQuickMode.class));
		if (quic_op == null) {
			logger.severe("Quick-Mode transition not possible, since QM operator is missing");
			return;
		}
		if (base_op == null) {
			logger.severe("Quick-Mode transition not possible, since Base operator is missing");
