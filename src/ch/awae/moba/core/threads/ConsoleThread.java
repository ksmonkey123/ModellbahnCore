...
public class ConsoleThread extends Thread {
	...
	private final Level[] levels = { Level.OFF, Level.SEVERE, Level.WARNING, Level.INFO, Level.CONFIG, Level.FINE,
		...
		while (true) {
			try {
				...
				if (command.equals("exit"))
					...
					System.out.println("current log level: " + logger.getLevel().getName());
	...
	private void listAllPaths() {
		...
		for (int i = 0; i < paths.length; i += 3) {
			...
			String line = strech(p1 == null ? "" : ((model.paths.isRegistered(p1) ? " * " : "   ") + p1.title), 20)
					+ strech(p2 == null ? "" : ((model.paths.isRegistered(p2) ? " * " : "   ") + p2.title), 20)
					+ strech(p3 == null ? "" : ((model.paths.isRegistered(p3) ? " * " : "   ") + p3.title), 20);
	...
	private void doLevelList() {
		...
		for (int i = 0; i < levels.length; i++)
			System.out.println(
					" #" + i + ": " + (levels[i].equals(logger.getLevel()) ? ">" : " ") + " " + levels[i].getName());
		System.out.println("====================");
	}
	...
	private void doList(String title, Registry<? extends Controllable> reg) {
		...
		for (int i = 0; i < names.size(); i++) {
			System.out.println(" #" + i + ": " + (reg.get(names.get(i)).isActive() ? ">" : " ") + " " + names.get(i));
		}
		System.out.println("===================");
	}
